...
# --- Код скрипта ---
def main():
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Загрузка VLESS конфигов...")
    ...
    if target_path.exists():
        print(f"✅ VLESS сохранен. Размер: {target_path.stat().st_size} байт")
    else:
        print("❌ Ошибка сохранения VLESS")
        sys.exit(1)
